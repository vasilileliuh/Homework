...
//            System.out.println(inputLine.replace("}", "}\n"));
        ...
        final LocalDate localStartDate = LocalDate.parse(startDate, dtf);
        final LocalDate localEndDate = LocalDate.parse(endDate, dtf);
        ...
        for (int i = 0; i < diff + 1; i++) {
            int arrayLength = data.getJSONObject("near_earth_objects")
                    .getJSONArray(start.toString()).length();
            for (int j = 0; j < arrayLength; j++) {
                asteroids.add(new Asteroid(start, getMissDistance(data, start, j), getDiameter(data, start, j), getIsHazardous(data, start, j)));
            }
    ...
    private void printResult(List<Asteroid> asteroids) {
        ...
        for (Asteroid asteroid : asteroids) {
            System.out.printf("%s - %.1fmln km from earth, %.2fkm diameter, %s%n",
